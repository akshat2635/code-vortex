...
function LanguageSelector({ hasAccess }: { hasAccess: boolean }) {
  ...
  const curLangObj = LANGUAGE_CONFIG[language];
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <motion.button
        ...
        ${!hasAccess && !["javascript", "python", "cpp"].includes(language) ? "cursor-not-allowed opacity-50" : ""}`}
      >
        {/* Decoration */}
        <div
          ...
          >
            ...
            <div className="max-h-[280px] overflow-y-auto overflow-x-hidden">
              {Object.values(LANGUAGE_CONFIG).map((lang, index) => {
                const isLocked =
                  !hasAccess &&
                  !["javascript", "python", "cpp"].includes(lang.id);
                return (
                  <motion.div
                    key={lang.id}
                    initial={{ opacity: 0 }}
